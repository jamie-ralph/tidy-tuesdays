...
selected_countries %>%
    ...
    mutate(is_known = case_when(
        parent_company %in% c("Unbranded", "null", "see excel file") ~ "unknown",
        TRUE ~ "known"
    )) %>%
...
plot_bottle_plot <- function(df, country_name, top_fill_radius) {
    ...
    ggplot() +
        geom_shape(data = bottle, aes(x = x, y = y), fill = NA, col = "#8d99ae",
                   expand = unit(-2, 'mm'), radius = unit(17.5, 'mm'), size = 1.5) +
        geom_shape(data = top, aes(x = x, y = y), fill = NA, col = "#8d99ae",
                   radius = unit(3, 'mm'), size = 1.5) +
        geom_shape(data = country_tab, aes(x = x, y = y), fill = "#00b4d8",
                   expand = unit(-2, 'mm'), radius = unit(17.5, 'mm')) +
        geom_shape(data = country_tab_extra, aes(x = x, y = y), fill = "#00b4d8",
...
france <- plot_bottle_plot(proportions_unknown_by_country, "France", 10)
...
most polluting brands. A large portion of collected plastic has no identifiable brand. 
This graphic explores the <span style = 'color:#00b4d8;'>proportion of unbranded plastic<br> waste</span> in a few select countries in 2020.\n\n",
        ...
        theme = theme(
            ...
            plot.caption = element_text(family = "Monda", colour = "#ef233c", size = 16, face = "bold"),
